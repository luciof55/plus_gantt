module PlusganttChartHelper
    # Simple class to handle gantt chart data
    class PlusganttChart  
      ...
      attr_reader :year_from, :month_from, :date_from, :date_to, :zoom, :months, :truncated, :max_rows, :control_date, :render_versions
      attr_accessor :query
      ...
      def initialize(options={})
        ...
        if options[:year] && options[:year].to_i >0
          ...
          if options[:month] && options[:month].to_i >=1 && options[:month].to_i <= 12
			...
			if options[:main_project] && options[:main_project].start_date
        ...
        if options[:months]
			...
			if User.current.pref[:gantt_months]
				months = (User.current.pref[:gantt_months]).to_i
			else
				if Plusgantt.months
					months = Plusgantt.months.to_i
				else
					months = 6
				end
			end
		...
		if options[:render_versions] && options[:render_versions] == '1'
			...
			@render_versions = false
		end
		@utils = Utils.new()
      end

      def common_params
        { :controller => 'plusgantt', :action => 'show', :project_id => @project }
      end

      def params
        common_params.merge({:zoom => zoom, :year => year_from,
                             :month => month_from, :months => months, :control_date => control_date,
      ...
      def params_previous
        common_params.merge({:year => (date_from << months).year,
                             ...
                             :zoom => zoom, :months => months, :control_date => control_date,
							 :render_versions => render_versions})
      end
      ...
      def projects
        ...
        if ids.any?
			...
			@projects = Project.visible.joins("LEFT JOIN #{Project.table_name} child ON #{Project.table_name}.lft <= child.lft AND #{Project.table_name}.rgt >= child.rgt").where("child.id IN (?)", ids).order("#{Project.table_name}.lft ASC").uniq.to_a
    ...
    def line(start_date, end_date, done_ratio, markers, label, options, object=nil, issues=nil)
      ...
      def to_image(format='PNG')
		...
		# control black line
        if control_date >= @date_from and control_date <= date_to
          gc.stroke('black')
          x = (control_date - @date_from + 1) * zoom + subject_width
          gc.line(x, headers_height, x, headers_height + g_height - 1)
        end
        gc.draw(imgl)
        imgl.format = format
	  ...
	  def render_issue_tooltip(issue)
		...
		@cached_label_total_estimated_hours ||= l(:field_total_estimated_hours)
		@cached_label_hours_per_day ||= l(:label_hours_per_day)
		@cached_label_assigned_to ||= l(:field_assigned_to)

		if issue.estimated_hours && (issue.total_estimated_hours > issue.estimated_hours)
    ...
    def html_subject_content(object)
        case object
			...
			when Project
				...
				onclick = "toggleProject('span-project-" + project.id.to_s + "', '" + project.id.to_s + "', null);return false;"
				s = view.link_to_project(project).html_safe
				view.content_tag(:span, s, :id => 'span-project-' + project.id.to_s, :class => html_class, :onclick => onclick).html_safe
		end
    end
    ...
    def html_subject(params, subject, object)
        ...
        case object
			when Issue
				issue = object
				css_classes = "project-" + issue.project.id.to_s + " pgissue-subject"
      ...
      def html_task(params, coords, markers, label, object)
		...
		if coords[:bar_start] && coords[:bar_end]
			...
			if object.is_a?(Issue)
				...
				css_class << " project-" + object.project.id.to_s
	...
	def generate_issue_bars(params, coords, object, css)
		output = ''
	...
	def render_bar(left, width, object, render_rel, css)	
		...
		if render_rel
			html_id = "task-line-issue-#{object.id}" if object.is_a?(Issue)
			html_id = "task-line-version-#{object.id}" if object.is_a?(Version)
			content_opt[:id] = html_id
			if object.is_a?(Issue)
		...
		return view.content_tag(:div, '&nbsp;'.html_safe, content_opt)
	end
